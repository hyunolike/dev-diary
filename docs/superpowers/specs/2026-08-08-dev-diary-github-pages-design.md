...
**어휘에 새로 추가하는 태그**는 최소 2편 이상에 붙어야 한다. 한 편에만 해당하는 개념을
어휘에 새로 들이면 필터가 잡음만 늘린다.

이미 어휘에 있는 태그는 1편에만 붙어도 된다. 그 글의 주제가 명백히 그것이라면 붙이는 편이
낫다 — 태그는 필터인 동시에 카드에 보이는 라벨이라, SQLite를 다룬 글에서 `SQLite`를 떼고
`아키텍처`로 대체하면 필터는 깔끔해져도 읽는 사람이 얻는 정보는 줄어든다. 초안에는 이
구분이 없어 실제로 그런 교체가 일어났다.
